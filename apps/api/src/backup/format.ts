...
const backupSchema = z.object({
  ...
  accountScope: accountScopeSchema,
  keyMetadata: z.object({ version: z.literal(1), fingerprint: z.string().min(1) }),
  nonce: z.string(),
  ciphertext: z.string(),
  authTag: z.string(),
})
...
export function createEncryptedBackup(
  ...
  try {
    ...
    const envelope = cipher.encrypt(JSON.stringify(parsed), { accountScope: parsed.accountScope })
    return {
