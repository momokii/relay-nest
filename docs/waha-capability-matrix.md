...
|---|---|---|
| WAHA image/version target | `devlikeapro/waha:latest-2026.8.1` | Published tag selected for the 2026.8.1 contract; the bundled Dockerfile pins its immutable digest |
| Immutable image digest | `sha256:d52ad4f394d2e48eb92d58e0f04924ff6c7621a883d08ff64176479ecd77c9ca` | Pulled and verified for the bundled runtime |
...
| Image runtime verification | Docker Compose bundled smoke startup with PostgreSQL, WAHA, API, and web | PASS for service startup, authenticated health, internal ports, and volume wiring; no linking or delivery claim |
...
| `deployment` | Dashboard-only and bundled-WAHA deployment | N/A — Docker image, internal network, and reverse-proxy configuration | Server secrets; no public WAHA API | Exact invocations use `docker-compose.yml` plus `docker-compose.override.yml` and either `docker-compose.external-waha.yml` or `docker-compose.bundled-waha.yml`; only web publishes a host port, API/WAHA remain internal; public access requires TLS/firewall hardening; external mode is verified with disposable placeholder-provider ordering/readiness/proxy/exposure; bundled profile builds the digest-pinned `latest-2026.8.1` image and uses the repository-owned Docker-secret wrapper, with local startup/health verified but no linking or delivery claim | https://waha.devlike.pro/docs/how-to/install/ | operational | external provider path and bundled local runtime |

### Sessions, health, and deployment

| Capability | Method / path | Auth scope | Engine caveat | Implementation / test status | Evidence |
|---|---|---|---|---|---|
| Ping | `GET /ping` | Exempt only if configured with the runtime-supported exclusion variable; otherwise `api_key` | Service liveness, not session readiness | RelayNest configures no exclusion; exact source/docs variable spelling requires runtime verification | [Observability](https://waha.devlike.pro/docs/how-to/observability/#ping) |
| Health | `GET /health` | `api_key` in OpenAPI; RelayNest configures no exclusion | Reports service/storage indicators, not WhatsApp account readiness; 200 healthy, 503 unhealthy | Bundled Compose authenticates the internal healthcheck from the mounted secret; health remains a service signal, not linking or delivery proof | [Observability](https://waha.devlike.pro/docs/how-to/observability/#health-check) |
...
| Deployment image | `devlikeapro/waha:latest-2026.8.1` pinned by digest; image families `latest`, `chrome`, `noweb`, `gows`, ARM variants | Image/secret configuration, not API scope | The dated `2026.8.1` tag has no manifest; the bundled wrapper pins and runs the published `latest-2026.8.1` image. Architecture support follows the published manifest and is not broadened by RelayNest | Implemented and locally smoke-tested for startup and authenticated health; no linking or delivery claim | [Engines](https://waha.devlike.pro/docs/how-to/engines/#docker-images), [Install](https://waha.devlike.pro/docs/how-to/install/#docker) |
...
6. **Version drift in examples:** Install, storage, and engine pages use unpinned `devlikeapro/waha`/`latest` examples. RelayNest uses the published `latest-2026.8.1` image pinned to the recorded digest above; this repository does not broaden support to other image families or architectures without a separate runtime check.
