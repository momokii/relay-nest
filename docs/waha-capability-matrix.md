...
| `deployment` | Dashboard-only and bundled-WAHA deployment | N/A — Docker image, internal network, and reverse-proxy configuration | Server secrets; no public WAHA API | Exact invocations use `docker-compose.yml` plus `docker-compose.override.yml` and either `docker-compose.external-waha.yml` or `docker-compose.bundled-waha.yml`; only web publishes a host port, API/WAHA remain internal; public access requires TLS/firewall hardening; external mode is verified only for disposable placeholder-provider ordering, readiness, proxy, and exposure; bundled profile is fail-closed because the exact image has no manifest and no supported secret boundary is verified | https://waha.devlike.pro/docs/how-to/install/ | operational | external-mode verified boundary; bundled blocked |
