...
| Timelock / capping | `GET /api/sessions/{session}/timelock`; `GET /api/sessions/{session}/capping` | `api_key`; docs support timelock in GOWS/NOWEB/WEBJS; capping is exposed in live 2026.8.1 contract. Read-only `read`. Do not restart: `463`/`475` are server-side outreach gates while session remains `WORKING`. | Not implemented / no repository test |
...
| `timelock-capping` | Outreach safety gates | GET /api/sessions/{session}/timelock; GET /api/sessions/{session}/capping | `api_key`; read | 463/475 are visible safety/recovery states; do not restart automatically | https://waha.devlike.pro/docs/overview/how-to-avoid-blocking/ | mvp | adapter-contract |
| `deployment` | Dashboard-only and bundled-WAHA deployment | N/A — Docker image, internal network, and reverse-proxy configuration | Server secrets; no public WAHA API | Pin image tag/digest; dashboard may bind `0.0.0.0`, WAHA API remains internal; public access requires TLS/firewall hardening | https://waha.devlike.pro/docs/how-to/install/ | operational | manual |

### Sessions, health, and deployment

| Capability | Method / path | Auth scope | Engine caveat | Implementation / test status | Evidence |
|---|---|---|---|---|---|
| Ping | `GET /ping` | Exempt only if configured with `WAHA_API_KEY_EXCLUDE_PATH`; otherwise `api_key` | Service liveness, not session readiness | Not implemented / no repository test | [Observability](https://waha.devlike.pro/docs/how-to/observability/#ping) |
| Health | `GET /health` | `api_key` in OpenAPI; may be excluded by config | Reports service/storage indicators, not WhatsApp account readiness; 200 healthy, 503 unhealthy | Not implemented / no repository test | [Observability](https://waha.devlike.pro/docs/how-to/observability/#health-check) |
...
7. **Health semantics:** `GET /health` is API-key protected in OpenAPI, while security docs show it may be excluded. Keep it internal or explicitly configure a narrow exemption; do not infer session connectivity from a 200 health response.
